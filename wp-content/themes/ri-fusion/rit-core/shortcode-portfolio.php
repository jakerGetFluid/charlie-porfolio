...
?>
<div id="<?php echo esc_attr($wrapID) ?>" class="rit-portfolio <?php echo esc_attr($atts['style'])?>"
    ...
    if ($the_query->have_posts()):
        if (!empty($terms) && !is_wp_error($terms)): ?>
            <div class="wrap-portfolio-filter">
                <ul id="portfolio-filter" class="col-xs-12 <?php echo esc_attr('align-'.$atts['filter_align'])?>">
                    <?php if (count($terms) > 1) : ?>
                        <li class="active" data-id="all"><span><?php echo esc_html__('All', 'ri-fusion') ?></span></li>
                        <?php foreach ($terms as $term) :
                            ?>
                            <li data-id="<?php echo esc_attr('id-'.$term->term_id) ?>">
            ...
            while ($the_query->have_posts()): $the_query->the_post();
                ...
                <article id="portfolio-<?php esc_attr(the_ID()); ?>" <?php echo post_class($class . $catslug); ?>
                    ...
                    <div class="wrap-portfolio-item">
                        <?php
                        if(get_post_meta(get_the_ID(),'rit_portfolio_format',true)=='embed'){
                            ?>
