...
<?php
    ...
    if($atts['show_filter']){
        ...
        //enable search
        if($atts['show_search']){
            echo '<label><input type="text" name="s" placeholder="Product Search"><input class="rit_search_button" type="button" value="Search"></label>';
            echo '<div class="rit_search_result"></div>';
            echo '<form style="display: none"  role="search" method="get" class="woocommerce-product-search" action="'.esc_url( home_url( '/'  ) ).'">
                    <label class="screen-reader-text" for="s">'.esc_html__( 'Search for:', 'rit-core-language' ).'</label>
                    <input type="search" class="search-field" placeholder="'. esc_attr_x( 'Search Products&hellip;', 'placeholder', 'rit-core-language' ).'" value="'. get_search_query().'" name="s" title="'. esc_attr_x( 'Search for:', 'label', 'rit-core-language' ).'" />
                    <input type="submit" value="'.esc_attr_x( 'Search', 'submit button', 'rit-core-language' ).'" />
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($){
                function rit_ajax_filter_<?php echo $varid; ?>(){
                    if(typeof filter_links_<?php echo $varid; ?> == 'undefined'){
                        ...
                        var search_button = wrap.find('.rit_search_button');
                        ...
                        var filter_links_<?php echo $varid; ?> = wrap.find('.rit-ajax-load a, .rit-remove-attribute');

                        filter_links_<?php echo $varid; ?>.click(function(e) {
                ...
                rit_ajax_filter_<?php echo $varid; ?>();
            }(jQuery));
...
$class = ''; ?>
    ...
    if(!isset($_POST['ajax'])):
        if($atts['show_loadmore'] && $product_query->max_num_pages > $atts['wc_attr']['paged']):
            echo '<a class="rit_ajax_load_more_button">'.esc_html__('Load more', 'rit-core-language').'</a>';
