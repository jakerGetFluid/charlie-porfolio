...
function rit_twitter_tweets()
{
    ...
    $TweetPHP = new TweetPHP(array(
        ...
        'tweet_wrap_close' => '</li>',
        'error_message' => esc_html__('Oops, our twitter feed is unavailable right now.', 'rit-core-language'),
        'error_link_text' => esc_html__('Follow us on Twitter', 'rit-core-language'),
        'debug' => false
    ));
...
class RIT_Twitter_Tweets_Widget extends WP_Widget
{
    ...
    {
        $widget_ops = array('classname' => 'twitter-tweets', 'description' => esc_html__('The most recent tweets from twitter.', 'rit-core-language'));

        $control_ops = array('id_base' => 'tweets-widget');

        parent::__construct('tweets-widget', esc_html__('RIT: Twitter Tweets', 'rit-core-language'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        if ($screen_name && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) {
            ...
            <div class="tweets-box">
                <p><?php esc_html_e('Please wait...', 'rit-core-language') ?></p>
            </div>
        ...
        <?php
            ...
            echo '<p>' . esc_html__('Please configure widget options.', 'rit-core-language') . '</p>';
        }
    ...
    {
        $defaults = array('title' => esc_html__('Latest Tweets', 'rit-core-language'), 'screen_name' => '', 'count' => 2, 'consumer_key' => '', 'consumer_secret' => '', 'access_token' => '', 'access_token_secret' => '');
        $instance = wp_parse_args((array)$instance, $defaults); ?>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
                <strong><?php echo esc_html__('Title', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('consumer_key')); ?>">
                <strong><?php echo esc_html__('Consumer Key', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('consumer_secret')); ?>">
                <strong><?php echo esc_html__('Consumer Secret', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>">
                <strong><?php echo esc_html__('Access Token', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('access_token_secret')); ?>">
                <strong><?php echo esc_html__('Access Token Secret', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('screen_name')); ?>">
                <strong><?php echo esc_html__('Twitter Screen Name', 'rit-core-language') ?>:</strong>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('count')); ?>">
                <strong><?php echo esc_html__('Number of Tweets', 'rit-core-language') ?>:</strong>
        ...
        <p><strong><?php echo esc_html__('Info', 'rit-core-language') ?>
                :</strong><br/><?php echo esc_html__('You can find or create <a href="http://dev.twitter.com/apps" target="_blank">Twitter App here</a>.', 'rit-core-language') ?>
        </p>
