...
if (!class_exists('RITRecentPosts')) {
    class RITRecentPosts extends WP_Widget
    {
        ...
        {
            $widget_ops = array('classname' => 'widget-recent-posts', 'description' => esc_html__('Show recent posts.', 'rit-core-language'));

            $control_ops = array('id_base' => 'recent_posts-widget');

            parent::__construct('recent_posts-widget', esc_html__('RIT: Recent Posts', 'rit-core-language'), $widget_ops, $control_ops);
        }
        ...
        {
            $defaults = array('title' => esc_html__('Recent Posts', 'rit-core-language'), 'number' => 6, 'items' => 3, 'view' => 'small', 'cat' => '', 'show_image' => 'on');
            $instance = wp_parse_args((array)$instance, $defaults); ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
                    <strong><?php esc_html_e('Title', 'rit-core-language') ?>:</strong>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>">
                    <strong><?php esc_html_e('Number of posts to show', 'rit-core-language') ?>:</strong>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('view')); ?>">
                    <strong><?php esc_html_e('View Type', 'rit-core-language') ?>:</strong>
                    <select class="widefat" id="<?php echo esc_attr($this->get_field_id('type')); ?>"
                            name="<?php echo esc_attr($this->get_field_name('view')); ?>">
                        <option
                            value="small"<?php echo (isset($instance['view']) && $instance['view'] == 'small') ? ' selected="selected"' : '' ?>><?php esc_html_e('Small', 'rit-core-language') ?></option>
                        <option
                            value="large"<?php echo (isset($instance['view']) && $instance['view'] == 'large') ? ' selected="selected"' : '' ?>><?php esc_html_e('Large', 'rit-core-language') ?></option>
                    </select>
                </label>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('items')); ?>">
                    <strong><?php esc_html_e('Number of items per slide', 'rit-core-language') ?>:</strong>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('cat')); ?>">
                    <strong><?php esc_html_e('Category IDs', 'rit-core-language') ?>:</strong>
            ...
            <p>
                ...
                <label
                    for="<?php echo esc_attr($this->get_field_id('show_image')); ?>"><?php echo esc_html__('Show Post Image', 'rit-core-language') ?></label>
            </p>
