...
if(!class_exists('RIT_Menu')){
	
	class RIT_Menu{
		public function __construct(){
			// add custom menu fields to menu
			add_filter( 'wp_setup_nav_menu_item', array($this, 'rit_add_custom_nav_fields') , 10, 3 );

			// save menu custom fields
			add_action( 'wp_update_nav_menu_item', array($this, 'rit_update_custom_nav_fields'), 10, 3 );

			// edit menu walker
			add_filter( 'wp_edit_nav_menu_walker', array($this, 'rit_menu_edit_walker'), 10, 2 );
		}

		public function rit_add_custom_nav_fields( $menu_item ) {
		...
		public function rit_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		...
		public function rit_menu_edit_walker($walker = '', $menu_id = '') {
		    return 'RIT_Walker_Nav_Menu_Edit';
		}
	}
...
class RIT_Walker_Nav_Menu_Edit extends Walker_Nav_Menu  {
    /**
    ...
    <p class="description description-wide">
        <label for="edit-menu-item-icon-<?php echo $item_id; ?>">
            ...
            <span><?php echo __('Input font awesome icon or icon class. You can see <a target="_blank" href="http://fortawesome.github.io/Font-Awesome/icons/">Font Awesome Icons in here</a>.', 'rit') ?></span>
        </label>
    </p>
    <p class="description">
...
/* Top Navigation Menu */
if (!class_exists('rit_top_navwalker')) {
    class rit_top_navwalker extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ($item->block)
                $item_output .= '<div class="menu-block menu-block-after">'.do_shortcode('[rit_block name="'.$item->block.'"]').'</div>';
...
/* Sidebar Menu */
if (!class_exists('rit_sidebar_navwalker')) {
    class rit_sidebar_navwalker extends Walker_Nav_Menu {
...
/* Accordion Menu */
if (!class_exists('rit_accordion_navwalker')) {
    class rit_accordion_navwalker extends Walker_Nav_Menu {
