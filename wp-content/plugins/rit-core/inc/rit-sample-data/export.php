...
if(!function_exists('rit_export_wp')){
    function rit_export_wp($args) {
...
<?php if ( $post_ids ) {
    ...
    $wp_query->in_the_loop = true;
    global $attachment_count_rit;
    $attachment_count_rit = 0;
    // Fetch 20 posts at a time rather than loading the entire table into memory.
    while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
    $where = 'WHERE ID IN (' . join( ',', $next_posts ) . ')';
    $posts = $wpdb->get_results( "SELECT * FROM {$wpdb->posts} $where" );
    if( $attachment_count_rit == 0){
        global $attachment_id;
        foreach ($posts as $key => $post_value) {
            if ( $post_value->post_type == 'attachment' ){
                $attachment_id = $post_value->ID;
                break;
            }
        }
    }
    ...
    foreach ( $posts as $post ) {
        ...
        if( $post->post_type == 'attachment' && $attachment_count_rit == 0):
?>
    <item>
        <title><?php
        ...
        ?></excerpt:encoded>
        <wp:post_id><?php echo $attachment_id; ?></wp:post_id>
...
<?php   if ( $post->post_type == 'attachment' ) : 
        global $attachment_count_rit;
            $attachment_count_rit = 1;

        if(file_exists( get_template_directory(). '/rit-core/placeholder.png' )):
                $attachment_url = get_template_directory_uri(). '/rit-core/placeholder.png';?>
        <wp:attachment_url><?php echo $attachment_url; ?></wp:attachment_url>
        <?php elseif(file_exists( get_template_directory(). '/rit-core/placeholder.jpg' )):
                $attachment_url = get_template_directory_uri(). '/rit-core/placeholder.jpg';?>
        <wp:attachment_url><?php echo $attachment_url; ?></wp:attachment_url>
        <?php else :?>
...
add_action('wp_ajax_rit_ajax_export_sample_data', 'rit_ajax_export_sample_data');
add_action('wp_ajax_nopriv_rit_ajax_export_sample_data', 'rit_ajax_export_sample_data');
if(!function_exists('rit_ajax_export_sample_data')){
    function rit_ajax_export_sample_data(){
