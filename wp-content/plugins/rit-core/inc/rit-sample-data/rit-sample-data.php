...
if(!function_exists('rit_import_sample_data_menu')){
    function rit_import_sample_data_menu() {
        add_management_page( esc_html__('RIT Import Sample Data', 'rit-core-language'), esc_html__('RIT Import Sample Data', 'rit-core-language'), 'manage_options', 'rit_import_sample_data', 'rit_import_sample_data' );
    }
}
...
if(!function_exists('rit_import_sample_data')){
    function rit_import_sample_data() {
        if ( !current_user_can( 'manage_options' ) )  {
            wp_die( __( 'You do not have sufficient permissions to access this page.', 'rit-core-language') );
        }
        ?>

        <div class="wrap">
            <h1><?php echo esc_html__( 'RIT Import Sample Data', 'rit-core-language'  ); ?></h1>
            <div class="rit-oneclick-install">
                <div class="rit-oneclick-install-overlay">
                    <div class="rit-overlay-icon">
                        ...
                        <span><?php echo esc_html(__('Please wait a moment', 'rit-core-language')); ?></span>
                    </div>
                </div>
                <div class="rit-base-install">
                    <h2><?php echo esc_html(__('Install demo content just in few clicks', 'rit-core-language')); ?></h2>
                    <input type="button" class="rit-install-base" value="<?php echo esc_html__('Install Base Demo Content', 'rit-core-language' ); ?>">
                </div>
                    ...
                    if( file_exists( RIT_SAMPLE_DATA_PATH . 'list.json' )){
                        ...
                        if(count($versions) > 0){
                            ...
                            <div class="rit-verison-type">
                                <h2><?php echo esc_html(__('Set up one of our theme versions', 'rit-core-language')); ?></h2>
                                <ul>
                                    <li class="active" data-filter="*"><?php echo esc_html__('All', 'rit-core-language')?></li>
                                    <?php
                                        $type = array();
                                        ...
                                        foreach ($type as $value_type) {
                                            ?>
                                                <li data-filter=".<?php echo esc_attr( $value_type); ?>" data-type="<?php echo esc_attr( $value_type); ?>">
                                                    <?php echo esc_html__( ucfirst( implode(' ', explode('-', $value_type)) ), 'rit-core-language')?>
                                                </li>
                                            <?php
                            ...
                            foreach ($versions as $key => $version) {
                                ?>
                                    <div class="rit-version-item <?php echo esc_attr( implode(' ', $version['type'])); ?>">
                                        ...
                                        <input type="button" data-version_path="<?php echo esc_attr( $version['path'] ); ?>" class="rit-install-verison" value="<?php echo esc_html__('Install', 'rit-core-language' ); ?>">
                                    </div>
                                <?php
                            }
                            ?>
...
if(!function_exists('rit_export_sample_data_menu')){
    function rit_export_sample_data_menu() {
        add_management_page( esc_html__('RIT Export Sample Data', 'rit-core-language'), esc_html__('RIT Export Sample Data', 'rit-core-language'), 'manage_options', 'rit_export_sample_data', 'rit_export_sample_data' );
    }
}
...
if(!function_exists('rit_export_sample_data')){
    function rit_export_sample_data() {
        ...
        <div class="wrap">
        <h1><?php echo esc_html__( 'RIT Export Sample Data', 'rit-core-language'  ); ?></h1>

        <p><?php _e('RIT Export sample data will replace all image with a placeholder image. Please put placeholder image to rit-core directory wp-content/theme/{theme_name}/rit-core/placeholder.png', 'rit-core-language' ); ?></p>
        ...
        <p><label><input type="radio" name="content" value="all" checked="checked" /> <?php _e( 'All content', 'rit-core-language'  ); ?></label></p>
        <p class="description"><?php _e( 'This will contain all of your posts, pages, comments, custom fields, terms, navigation menus and custom posts.' , 'rit-core-language' ); ?></p>

        <p><label><input type="radio" name="content" value="posts" /> <?php _e( 'Posts', 'rit-core-language' ); ?></label></p>
        <ul id="post-filters" class="export-filters">
            <li>
                <label><?php _e( 'Categories:' ); ?></label>
                <?php wp_dropdown_categories( array( 'show_option_all' => __('All', 'rit-core-language' ) ) ); ?>
            </li>
            <li>
        ...
        ?>
            ...
            <li>
                ...
                <select name="post_start_date">
                    <option value="0"><?php _e( 'Start Date', 'rit-core-language'  ); ?></option>
                    <?php export_date_options(); ?>
                </select>
                <select name="post_end_date">
                    <option value="0"><?php _e( 'End Date', 'rit-core-language'  ); ?></option>
                    <?php export_date_options(); ?>
                </select>
            </li>
            <li>
                ...
                <select name="post_status">
                    <option value="0"><?php _e( 'All', 'rit-core-language'  ); ?></option>
        ...
        <p><label><input type="radio" name="content" value="pages" /> <?php _e( 'Pages', 'rit-core-language'  ); ?></label></p>
        <ul id="page-filters" class="export-filters">
            <li>
                <label><?php _e( 'Authors:' , 'rit-core-language' ); ?></label>
        <?php
        ...
        ?>
            ...
            <li>
                <label><?php _e( 'Date range:', 'rit-core-language'  ); ?></label>
            ...
            <li>
                <label><?php _e( 'Status:', 'rit-core-language'  ); ?></label>
        ...
        <?php submit_button( __('Download Export File', 'rit-core-language') ); ?>
        </form>
        </div>
        <script type="text/javascript">
...
if( !function_exists('rit_install_sample_data') ){
    ...
    function rit_install_sample_data(){
        ...
        if(is_dir( RIT_SAMPLE_DATA_PATH . $_POST['rit_path'])){
            ...
            if( count( glob( RIT_SAMPLE_DATA_PATH . $_POST['rit_path'] . "/megamenu*.json") ) ){
                foreach( glob( RIT_SAMPLE_DATA_PATH . $_POST['rit_path'] . "/megamenu*.json") as $menu){
                    if(file_exists($menu)){
                        rit_import_megamenu($menu);
                    }
                } 
            }       
        }
